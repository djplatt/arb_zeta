...
Code to do rigorous upsampling.
See 4.2 of
...
// first time we call this (sincp) we compute sin and cos
// from then on (not sincp) just swap signs each time
...
// returns f(t) in f_res, f'(t) in fd_res (unless NULL)
